...
if isempty(pad),  pad = 2.^max(nextpow2(window),14); end; % 
...
for i = 1:length(brkpts)-1
    windowedData(brkpts(i):brkpts(i+1),:) = detrend(windowedData(brkpts(i):brkpts(i+1),:)')'; % Detrend after windowing the data
end

tfse = zeros(brkpts(end),nFreqs);
%dof = tfse;
...
for i = 1:length(brkpts)-1
    ...
    if ((doAdapt && Ktapers>1) && ~isempty(concentration))
        ...
        for t = 1:size(dft,1)
            ...
            temp=zeros(nFreqs,1);
            S1=zeros(nFreqs,1);
            while sum(abs(S0-S1)/nFreqs)>tol
                b=(S0*ones(1,Ktapers))./(S0*concentration'+ones(nFreqs,1)*mser'); % Percival & Walden p368.
                wk=(b.^2).*(ones(nFreqs,1)*concentration'); 
            ...
            %dof(brkpts(i)+t-1,:) = 2*sum(wk.^2,2)./sum((b.^4).*(ones(nFreqs,1)*concentration'),2);
...
out.tfse = reshape(tfse,[nTrials,nSteps,nFreqs]);
if includePhase
    out.phase = reshape(phase,[nTrials,nSteps,nFreqs]);
end
