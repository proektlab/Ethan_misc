...
%   data: matrix of [trials samples]
    ...
    %McWu Note: channel can substitute for trial, given the output gives total
    %power as well

%   taper: matrix of [samples tapers] (note that this is the default output
%       format from the dpss function)

    % McWu note: for dpss, NW (sample length*half bandwidth) is usually a
    % constant that varies from 2.5 to 4. Ex. if you sample at 500 hz and you
    % want a half bandwidth of 2 hz, your half bandwidth is 1/250, and your
    % length could be 625 samples (for a ratio of 2.5, thus smoothing over ~1.2
    % seconds) to 1000 samples (for a ratio of 4, thus smoothing over 2 seconds)
    %
    %Additional possibility- define slepian sequences in Hz by dividing NW
    %by Freq for dpss
...
%   sample_step: the time in seconds between sample steps; default is 0.001
    ...
    %McWu note: need to figure out whether everything is determined by
    %sample or seconds. Unclear.
